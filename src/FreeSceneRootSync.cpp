...
namespace OStimTogether
{
    ...
    bool FreeSceneRootSync::HandleIncoming(
        STRPMApi::ConnectionID senderConnectionID,
        std::string_view payload)
    {
        if (!payload.starts_with("ROOTBONE|")) {
            return false;
        }

        if (!_threads || senderConnectionID == 0) {
            return true;
        }
    ...
    void FreeSceneRootSync::Tick()
    {
        ...
        if (player && ThreadContainsActor(thread, player)) {
            if (auto* animatedRoot = FindAnimatedRoot(player)) {
                if (_lastSend.time_since_epoch().count() == 0 ||
                    ...
                    if (local.IsFinite()) {
                        ...
                        STRPMTransport::GetSingleton().SendRealtime(
                            fmt::format(
                        ...
                        if (_lastSendLog.time_since_epoch().count() == 0 ||
                            now - _lastSendLog >= kLogInterval) {
                            _lastSendLog = now;
                            auto* node = thread->getCurrentNode();
                            SKSE::log::info(
                                "OSTNET ROOT SYNC TX thread={} node={} local=({:.3f},{:.3f},{:.3f}) referenceWrites=0",
                                _activePlayerThreadID,
